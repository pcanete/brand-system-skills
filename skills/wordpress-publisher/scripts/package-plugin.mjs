...
export async function packagePlugin({ pluginDir, outPath }) {
  ...
  if (files.length > 65535) throw new Error('El paquete excede 65.535 archivos y requiere ZIP64.');
  ...
  for (const relative of files) {
    ...
    if (content.length > 0xffffffff || payload.length > 0xffffffff || offset > 0xffffffff) {
      throw new Error('El paquete excede los límites ZIP clásicos. Reducí assets o usá ZIP64.');
    }
