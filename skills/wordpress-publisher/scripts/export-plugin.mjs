...
export function resolveConfig(raw = {}) {
  ...
  const cleanHeader = (label, value) => {
    const clean = String(value ?? '').trim();
    if (/\r|\n|<\?php|\?>/i.test(clean)) {
      throw new Error(`config.${label} contiene caracteres no permitidos.`);
    }
    return clean.slice(0, 240);
  };
  const constPrefix = raw.constPrefix || base.toUpperCase();
  const fnPrefix = raw.fnPrefix || base;
  if (!/^[A-Z][A-Z0-9_]*$/.test(constPrefix)) {
    throw new Error('config.constPrefix debe ser un identificador PHP en mayúsculas.');
  }
  if (!/^[a-z][a-z0-9_]*$/.test(fnPrefix)) {
    throw new Error('config.fnPrefix debe ser un identificador PHP en minúsculas.');
  }
  ...
  const extraSources = Array.isArray(raw.additionalStyleSources) ? raw.additionalStyleSources : [];
  const blockedStyleSources = [...new Set([
    ...defaultStyleSources,
    ...(raw.isolateWooStyles
      ? ['/plugins/woocommerce/assets/', '/plugins/woocommerce/packages/woocommerce-blocks/']
      : []),
    ...extraSources,
  ])];
  if (blockedStyleSources.some((value) =>
    typeof value !== 'string' || !value.startsWith('/') || /['"\r\n]/.test(value))) {
    throw new Error('additionalStyleSources sólo admite fragmentos de ruta absolutos y seguros.');
  }

  return {
    slug,
    version,
    name: cleanHeader('name', raw.name || slug),
    description: cleanHeader('description', raw.description || `Portada compilada para ${slug}.`),
    author: cleanHeader('author', raw.author || ''),
    constPrefix,
    fnPrefix,
    blockedStyleSources,
    headOwnership: raw.headOwnership === 'compiled' ? 'compiled' : 'wordpress',
  };
}
...
function render(source, config) {
  return source
    ...
    .replaceAll(
      '{{BLOCKED_STYLE_SOURCES}}',
      config.blockedStyleSources.map((value) => `'${value.replaceAll("'", "\\'")}',`).join('\n\t\t'),
    );
}
...
function stripWordPressOwnedHead(head, ownership) {
  let output = head
    .replace(/<meta\s+charset=[^>]*>/gi, '')
    .replace(/<meta\s+name=["']viewport["'][^>]*>/gi, '')
    .replace(/<title>[\s\S]*?<\/title>/gi, '');
  if (ownership === 'wordpress') {
    output = output
...
const ROOT_ASSET = /\/((?:assets|_astro)\/[A-Za-z0-9._~!$&+;=:@%\/?-]+)/g;

function rewriteAssetUrls(fragment, config) {
  const rewritten = fragment.replace(
    ROOT_ASSET,
    (_match, assetPath) => toPhpAssetUrl(assetPath, config),
  );
...
async function verifyAssetReferences(html, distDir) {
  const references = [...html.matchAll(ROOT_ASSET)].map((match) => match[1].split('?')[0]);
...
export async function exportPlugin({ projectRoot, config }) {
  ...
  const compiledHead = rewriteAssetUrls(stripWordPressOwnedHead(originalHead, config.headOwnership), config);
  ...
  const manifestItems = assetReferences
    .map((asset) => `\t'${asset.replaceAll("'", "\\'")}',`)
    .join('\n');
  await writeFile(
    path.join(pluginDir, 'required-assets.php'),
  ...
  const report = {
    ...
    blockedStyleSources: config.blockedStyleSources,
  };
