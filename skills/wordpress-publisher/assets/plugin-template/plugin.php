...
function {{fn_prefix}}_activate() {
	$required = array(
		...
		{{CONST_PREFIX}}_PATH . 'required-assets.php',
	);
	...
	$assets = require {{CONST_PREFIX}}_PATH . 'required-assets.php';
	foreach ( $assets as $asset ) {
		if ( ! is_file( {{CONST_PREFIX}}_PATH . 'dist/' . $asset ) ) {
			deactivate_plugins( plugin_basename( __FILE__ ) );
			wp_die( esc_html__( 'El paquete no contiene todos los assets declarados.', '{{slug}}' ) );
		}
	}
}
